...
class FroggyPriceNegociatorAjaxRequestValidatePriceProcessor extends FroggyHookProcessor
{
	...
	public function sendReminderMail()
	{
		...
		$templateVars = array(
			...
			'{expiration_date}' => FroggyDisplayDate($this->date_expiration, $this->id_lang, true),
	...
	public function run()
	{
		...
		// Check if price exists for this combination (since it is cast in float, it become 0.0, so we have to check numeric)
		if ($this->new_price < 0.1)
			return $this->params['ajaxController']->render('error', '');
